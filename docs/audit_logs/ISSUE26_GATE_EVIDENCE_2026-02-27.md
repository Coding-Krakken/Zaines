...
3. src/app/api/auth/magic-link/**tests**/route.test.ts contains duplicated test blocks and fails all contract assertions with `POST is not a function`.
4. src/app/api/booking/availability/**tests**/route.test.ts fails all assertions with `POST is not a function`.
5. src/app/api/contact/submissions/**tests**/route.test.ts fails with `__resetIssue26InMemoryState is not a function` (test/runtime module export linkage failure).
...
- Legacy availability route suite (non-issue26 endpoint) was run separately and passed:
  - src/app/api/availability/**tests**/route.test.ts (5 passed)
