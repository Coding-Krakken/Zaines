...
describe('notifications helper (resend path)', () => {
  ...
  it('sends via Resend when API key present', async () => {
    ...
    const booking: TestBooking = { id: 'b1', bookingNumber: 'PB-1', status: 'pending', user: { email: 'a@b.com', name: 'A' } };
  ...
  it('retries transient failures and succeeds', async () => {
    ...
    const booking: TestBooking = { id: 'b2', bookingNumber: 'PB-2', status: 'pending', user: { email: 'x@y.com', name: 'X' } };
    const res = await sendBookingConfirmation(booking);
    expect(res.provider).toBe('resend');
  ...
  it('sends payment notification via Resend', async () => {
    ...
    const booking: TestBooking = { id: 'b3', bookingNumber: 'PB-3', user: { email: 'pay@x.com' } };
