...
import {
  ...
  POST as postActivities,
} from "@/app/api/admin/activities/route";
import {
  GET as getPhotos,
  POST as postPhotos,
} from "@/app/api/admin/photos/route";
...
function makeRequest(
  ...
  url = "http://localhost/api/admin/check-in",
) {
...
describe("POST /api/admin/check-out", () => {
  ...
  it("returns 401 when not authenticated", async () => {
    ...
    const res = await postCheckOut(
      makeRequest(
        { bookingId: "book-1" },
        "http://localhost/api/admin/check-out",
      ),
    );
    expect(res.status).toBe(401);
  });
  ...
  it("returns 400 when bookingId is missing", async () => {
    authMock.mockResolvedValue(staffSession);
    const res = await postCheckOut(
      makeRequest({}, "http://localhost/api/admin/check-out"),
    );
    expect(res.status).toBe(400);
  });
...
describe("POST /api/admin/activities", () => {
  ...
  it("returns 403 for customer role", async () => {
    ...
    const req = makeRequest(
      { bookingId: "book-1", petId: "pet-1", type: "walk" },
      "http://localhost/api/admin/activities",
    );
...
describe("GET /api/admin/activities", () => {
  ...
  it("returns 401 when not authenticated", async () => {
    ...
    const res = await getActivities(
      new NextRequest("http://localhost/api/admin/activities"),
    );
    expect(res.status).toBe(401);
  });
  ...
  it("returns activities for staff role", async () => {
    ...
    const res = await getActivities(
      new NextRequest("http://localhost/api/admin/activities?limit=5"),
    );
    expect(res.status).toBe(200);
    const data = await res.json();
...
describe("POST /api/admin/photos", () => {
  ...
  function makePhotoRequest(fields: Record<string, string>) {
    ...
    return new NextRequest("http://localhost/api/admin/photos", {
      method: "POST",
      body: form,
    });
  }
  ...
  it("returns 409 when booking is not checked_in", async () => {
    ...
    const res = await postPhotos(
      makePhotoRequest({ bookingId: "book-1", petId: "pet-1" }),
    );
    expect(res.status).toBe(409);
    const data = await res.json();
...
describe("GET /api/admin/contacts", () => {
  ...
  it("returns 404 when booking filter does not exist", async () => {
    ...
    const res = await getContacts(
      new NextRequest("http://localhost/api/admin/contacts?bookingId=missing"),
    );
    expect(res.status).toBe(404);
  });
