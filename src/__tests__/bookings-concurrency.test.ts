  ...
  const mockPrisma = {
    $transaction: vi.fn(async (callback: TransactionCallback) => {
      ...
      const tx = {
        ...
        booking: {
          count: vi.fn(
            (args: { where: { suite: { tier: { equals: string } } } }) => {
...
describe("Booking Concurrency Safety", () => {
  ...
  it("prevents overbooking when 20 concurrent requests target same suite/dates", async () => {
    ...
    const requests = Array(20)
      .fill(null)
      .map(() => createBookingRequest("standard"));
    ...
    const rejectionCount = statuses.filter(
      (s) => s.status === 404 || s.status === 409,
    ).length;
  ...
  it("allows exactly 10 bookings for standard tier (capacity=10)", async () => {
    ...
    const rejectionCount = results.filter(
      (r) => r.status === 404 || r.status === 409,
    ).length;
  ...
  it("handles concurrent bookings across different suite tiers independently", async () => {
    const standardRequests = Array(5)
      .fill(null)
      .map(() => createBookingRequest("standard"));
    const deluxeRequests = Array(5)
      .fill(null)
      .map(() => createBookingRequest("deluxe"));
    const luxuryRequests = Array(3)
      .fill(null)
      .map(() => createBookingRequest("luxury"));
  ...
  it("returns 409 on transaction conflict with Retry-After header", async () => {
    ...
    const conflictError = new Error("Transaction conflict") as Error & {
      code: string;
    };
  ...
  it("properly isolates suite type locks", async () => {
    ...
    const standardPromises = Array(CAPACITY_STANDARD)
      .fill(null)
      .map(() => bookingsPost(createBookingRequest("standard") as NextRequest));

    // While those are "processing", deluxe should still work
    const deluxePromises = Array(CAPACITY_DELUXE)
      .fill(null)
      .map(() => bookingsPost(createBookingRequest("deluxe") as NextRequest));
