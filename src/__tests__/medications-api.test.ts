...
describe('POST /api/medications', () => {
  ...
  it('returns 401 when not authenticated', async () => {
    ...
    const res = await postMedication(makePostRequest({ petId: 'pet-1', ...validMedicationBody }));
  ...
  it('returns 403 when pet belongs to another user', async () => {
    ...
    const res = await postMedication(makePostRequest({ petId: 'pet-2', ...validMedicationBody }));
...
describe('DELETE /api/medications/[id]', () => {
  ...
  it('returns 401 when not authenticated', async () => {
    authMock.mockResolvedValue(null);
    const req = new NextRequest('http://localhost/api/medications/med-1', { method: 'DELETE' });
  ...
  it('deletes medication successfully', async () => {
    authMock.mockResolvedValue(userSession);
    prismaMock.medication.findUnique.mockResolvedValue({ ...storedMedication, pet: { userId: 'user-1' } });
