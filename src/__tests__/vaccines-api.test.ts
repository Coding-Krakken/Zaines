...
import { GET as getVaccines, POST as postVaccine } from '@/app/api/vaccines/route';
import {
  GET as getVaccine,
  PUT as putVaccine,
...
function makePostRequest(body: Record<string, unknown>, url = 'http://localhost/api/vaccines') {
...
function makeDeleteRequest() {
  return new NextRequest('http://localhost/api/vaccines/vaccine-1', { method: 'DELETE' });
}
...
describe('GET /api/vaccines', () => {
  ...
  it('filters vaccines by petId', async () => {
    authMock.mockResolvedValue(userSession);
    prismaMock.vaccine.findMany.mockResolvedValue([storedVaccine]);
    const res = await getVaccines(makeGetRequest('http://localhost/api/vaccines?petId=pet-1'));
    expect(res.status).toBe(200);
...
describe('POST /api/vaccines', () => {
  ...
  it('returns 401 when not authenticated', async () => {
    authMock.mockResolvedValue(null);
    const res = await postVaccine(makePostRequest({ petId: 'pet-1', ...validVaccineBody }));
    expect(res.status).toBe(401);
  });
  ...
  it('returns 403 when pet belongs to another user', async () => {
    ...
    const res = await postVaccine(makePostRequest({ petId: 'pet-2', ...validVaccineBody }));
...
describe('PUT /api/vaccines/[id]', () => {
  ...
  it('returns 401 when not authenticated', async () => {
    authMock.mockResolvedValue(null);
    const res = await putVaccine(makePutRequest(validVaccineBody), ctx('vaccine-1'));
    expect(res.status).toBe(401);
  });
  ...
  it('updates vaccine successfully', async () => {
    authMock.mockResolvedValue(userSession);
    prismaMock.vaccine.findUnique.mockResolvedValue({ ...storedVaccine, pet: { userId: 'user-1' } });
