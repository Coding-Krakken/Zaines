...
export function ContactSubmissionForm() {
  ...
  const [submissionState, setSubmissionState] =
    useState<ContactSubmissionState>("draft");
  ...
  const onSubmit = async (values: ContactSubmissionInput) => {
    ...
    try {
      ...
      const body = (await response
        .json()
        .catch(() => ({}))) as ContactSuccessResponse & ContactErrorResponse;
      ...
      if (
        body.errorCode === "CONTACT_RATE_LIMITED" ||
        response.status === 429
      ) {
      ...
      if (
        body.errorCode === "CONTACT_VALIDATION_FAILED" ||
        response.status === 422
      ) {
      ...
      form.setError("root", {
        message:
          body.message ||
          "We couldn't submit your message right now. Please retry.",
      });
    } catch {
      setSubmissionState("submit_failed_retryable");
      form.setError("root", {
        ...
        {submissionState === "submitted" ? (
          ...
          <form className="space-y-4" onSubmit={form.handleSubmit(onSubmit)}>
            {rootError && (
              <Alert
                variant={
                  submissionState === "throttled" ? "default" : "destructive"
                }
                role="alert"
                aria-live="assertive"
              >
