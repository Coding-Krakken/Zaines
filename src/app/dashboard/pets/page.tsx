...
export default async function PetsPage() {
  ...
  const pets = await prisma.pet.findMany({ where: { userId: session.user.id }, orderBy: { createdAt: 'desc' } });

  return (
    <div className="container mx-auto p-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold">My Pets</h1>
        <Link href="/dashboard/pets/new" className="btn">Add Pet</Link>
      </div>
      ...
      <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        ...
        {pets.map(p => (
          <div key={p.id} className="p-4 border rounded">
            ...
            <div className="mt-3">
              <Link href={`/dashboard/pets/${p.id}`} className="text-sm text-primary">View</Link>
            </div>
          </div>
        ))}
      </div>
